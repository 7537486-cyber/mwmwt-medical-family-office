...
当前联系页精简：用户发现联系页左侧咨询范围与右侧表单咨询方向重复，已将左侧项目列表删除，保留微信咨询入口，右侧表单作为唯一正式项目选择位置。
...
- 已继续优化首页信息结构：清除已不展示的旧价值模型/全球网络数据，避免未来误恢复重复区块；四大核心服务卡片现在直接链接到长寿规划、再生医疗、企业家健康和全球医疗资源内页；“客户为什么选择我们”五项也分别链接到对应承接页面。
- 已在联系表单加入性别与年龄段选项：中文、日文、英文分别显示对应文案；提交后内部邮件、CRM Webhook Payload 与 LINE 通知均包含 Gender / Age 字段。
...
- 已执行 `npm run build`，Next.js 生产构建通过，确认首页入口链接化与旧重复数据清理后仍正常生成 43 个路由
- 已执行 `npm run build`，Next.js 生产构建通过，确认联系表单新增性别与年龄字段后仍正常生成 43 个路由
